...
try {
  const testConnection = new Deno.Command("deno", {
    args: ["run", "--allow-net", "--allow-env", "--allow-read", "src/test-connection.ts"],
  });
  ...
  console.log("📝 Asegúrate de que la variable DATABASE_URL está configurada correctamente en el archivo .env");
  Deno.exit(1);
}
...
try {
  const migrate = new Deno.Command("deno", {
    args: ["run", "--allow-net", "--allow-env", "--allow-read", "src/db/migrate.ts"],
  });
...
try {
  const initAdmin = new Deno.Command("deno", {
    args: ["run", "--allow-net", "--allow-env", "--allow-read", "src/scripts/init-admin.ts"],
  });
