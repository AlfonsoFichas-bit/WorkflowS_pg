...
export const handler: Handlers = {
	async POST(req, _ctx) {
		try {
			...
			if (!rubricId) {
				return new Response(
					JSON.stringify({ error: "El ID de la rúbrica es obligatorio" }),
					{
						status: 400,
			...
			if (!name) {
				return new Response(
					JSON.stringify({ error: "El nombre del criterio es obligatorio" }),
					{
						status: 400,
						headers: { "Content-Type": "application/json" },
					},
			...
			if (rubric[0].creatorId !== user.id && user.role !== "admin") {
				return new Response(
					JSON.stringify({
						error: "No tienes permisos para añadir criterios a esta rúbrica",
					}),
					{
						status: 403,
						headers: { "Content-Type": "application/json" },
			...
			return new Response(
				JSON.stringify({ success: true, criterion: newCriterion[0] }),
				{
					status: 201,
					headers: { "Content-Type": "application/json" },
				},
			...
			return new Response(
				JSON.stringify({ error: "Error al crear el criterio" }),
				{
					status: 500,
					headers: { "Content-Type": "application/json" },
				},
	...
	async PUT(req, _ctx) {
		try {
			...
			if (!id || isNaN(Number(id))) {
				return new Response(
					JSON.stringify({ error: "ID de criterio inválido" }),
					{
						status: 400,
						headers: { "Content-Type": "application/json" },
					},
			...
			if (!criterion || criterion.length === 0) {
				return new Response(
					JSON.stringify({ error: "Criterio no encontrado" }),
					{
						status: 404,
						headers: { "Content-Type": "application/json" },
					},
			...
			if (rubric[0].creatorId !== user.id && user.role !== "admin") {
				return new Response(
					JSON.stringify({
						error: "No tienes permisos para modificar este criterio",
					}),
					{
						status: 403,
						headers: { "Content-Type": "application/json" },
			...
			const updatedCriterion = await updateRubricCriterion(
				criterionId,
				updateData,
			);

			return new Response(
				JSON.stringify({ success: true, criterion: updatedCriterion[0] }),
				{
					status: 200,
					headers: { "Content-Type": "application/json" },
				},
			...
			return new Response(
				JSON.stringify({ error: "Error al actualizar el criterio" }),
				{
					status: 500,
					headers: { "Content-Type": "application/json" },
				},
	...
	async DELETE(req, _ctx) {
		try {
			...
			if (rubric[0].creatorId !== user.id && user.role !== "admin") {
				return new Response(
					JSON.stringify({
						error: "No tienes permisos para eliminar este criterio",
					}),
					{
						status: 403,
						headers: { "Content-Type": "application/json" },
			...
			return new Response(
				JSON.stringify({ success: true, criterion: deletedCriterion[0] }),
				{
					status: 200,
					headers: { "Content-Type": "application/json" },
				},
			...
			return new Response(
				JSON.stringify({ error: "Error al eliminar el criterio" }),
				{
					status: 500,
					headers: { "Content-Type": "application/json" },
				},
	...
	async GET(req, _ctx) {
		try {
			...
			if (id) {
				...
				return new Response(
					JSON.stringify({ success: true, criterion: criterion[0] }),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
					},
			...
			return new Response(
				JSON.stringify({
					error: "Se requiere un ID de criterio o un ID de rúbrica",
				}),
				{
					status: 400,
					headers: { "Content-Type": "application/json" },
			...
			return new Response(
				JSON.stringify({ error: "Error al obtener criterios" }),
				{
					status: 500,
					headers: { "Content-Type": "application/json" },
				},
