...
interface Project {
	...
	createdAt: Date | null;
	updatedAt: Date | null;
	members?: Member[]; // Miembros del proyecto
}
...
export const handler: Handlers<ProjectsData, State> = {
	...
	async POST(req, ctx) {
		try {
			...
			return new Response(
				JSON.stringify({ error: "Error al crear el proyecto" }),
				{
					status: 500,
					headers: { "Content-Type": "application/json" },
				},
...
export const addUserToProjectHandler: Handlers = {
	async POST(req, ctx) {
		try {
			...
			return new Response(
				JSON.stringify({ success: true, teamMember: teamMember[0] }),
				{
					status: 200,
					headers: { "Content-Type": "application/json" },
				},
			...
			return new Response(
				JSON.stringify({ error: "Error al agregar usuario al proyecto" }),
				{
					status: 500,
					headers: { "Content-Type": "application/json" },
				},
...
export default function Projects({ data }: PageProps<ProjectsData>) {
	...
	return (
		<DashboardLayout user={user}>
			<div class="p-6">
				<ProjectsPageIsland projectsList={projectsList} />
			</div>
		</DashboardLayout>
	);
}
