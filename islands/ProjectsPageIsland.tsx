...
export default function ProjectsPageIsland({
	...
	const handleAddUserToProject = async (userData: {
		userId: number;
		role: string;
	}) => {
		...
		try {
			const response = await fetch(
				`/dashboard/projects/${selectedProject.id}/users`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			alert(
				"Usuario agregado/actualizado exitosamente en el proyecto. Recuerda que este rol es específico para este proyecto y no cambia el rol global del usuario.",
			);
		} catch (error) {
			console.error("Error:", error);
			alert(
	...
	const handleDeleteProject = async (projectId: number) => {
		if (
			!confirm(
				"¿Estás seguro de que deseas eliminar este proyecto? Esta acción no se puede deshacer y los roles de los usuarios volverán a 'team_developer'.",
			)
		) {
			return;
		}
